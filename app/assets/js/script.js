$(document).ready( () => {
	...
	$('.stars-outer').each(function(i, element){
		...
		let id = $(this).attr('id');
		$('#stars-inner'+id)[0].style.width = (rating/5*100) + '%';
	});
		...
		if(!enabled) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					$('.wish_heart').html("<i class='fas fa-2x text-red'></i>");
	...
	$(document).on('click', '#btn_order_confirmation', function(e){
		...
		if(addressId.length == 0) {	
			...
			$.post('../controllers/process_save_billing_address.php', {
				addressId: addressId
			}, function(response2){
	...
	$(document).on('click', '#btn_deactivate', function(e){
		...
		if(answer == true) {
			$.post('../controllers/process_deactivate_by_user.php', function(response){
				if(response == 'success' ){
